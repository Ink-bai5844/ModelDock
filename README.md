...
- 代码模式不是单纯的输入框样式：开关会按对话独立保存，并随每次聊天请求传到服务端，由服务端为模型注入完整实现、文件结构、运行说明和禁止虚构执行结果等编程规则。
- 未开启 Agent 时，代码模式要求模型按“文件名 + 带语言标记的完整代码块”输出，方便直接复制；开启 Agent 后，代码文件会真实写入当前账号工作区，并自动打包为聊天中的 ZIP 附件。
- 解释、诊断或代码审查类问题不会因为代码模式而强制生成文件；切换历史对话时会恢复该对话自己的代码模式状态。
- 模型输出的图片显示在聊天记录中；点击图片会打开全图预览，下载操作位于预览窗口内。
- 模型目录启用“深度思考模式”后，聊天输入框会出现“深度思考”按钮；开关状态按对话独立保存，重新进入历史对话时自动恢复，新建对话默认关闭。
...
| 模式 | 新建对话 | 重新打开历史对话 | 实际请求行为 |
| --- | --- | --- | --- |
| 深度思考 | 默认关闭 | 恢复该对话上次选择 | 仅当前模型声明支持深度思考时生效；不支持时隐藏按钮且不会向上游开启推理 |
| 代码模式 | 默认关闭 | 恢复该对话上次选择 | 服务端注入完整工程实现规则；与 Agent 同时开启时使用工作区文件和 ZIP 交付 |
| Agent | 沿用当前工作台选择，并在首次发送后归入新对话 | 恢复该对话上次选择 | 仅模型目录声明支持 Agent 时生效 |
| 联网搜索 | 跟随 Agent 状态 | 恢复该对话上次选择 | 只有 Agent 已开启时才能调用搜索工具 |
| Skill | 清空本轮手动选择 | 恢复该对话的候选 Skill 和账号调用策略 | “始终调用、智能判断、仅手动”策略由服务端逐轮执行 |

模式开关保存在对应会话的元数据中，不会写进用户消息正文。切换开关会进入统一的串行防抖保存队列；正在流式生成时，浏览器只更新内存状态，结束、停止或报错后再提交最终会话状态。
...
- 助手生成的 ZIP、图片、音视频等附件继续保留在聊天记录和工作区供预览或下载，但后续对话只会把用户输入附件还原并发送给上游，避免把助手输出文件错误编码成下一轮多模态输入。
...
Agent 会区分“创建或修改文件”和“回顾已有文件”：用户询问上次代码的具体内容、要求解释或确认已有实现时，可以读取工作区文件后直接回答，不会因为句子中出现“写的代码”而再次强制生成 ZIP；只有明确要求新建、修改、重写、补全、重新发送或导出文件时，才启用强制文件交付检查。
